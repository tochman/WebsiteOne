...
Given(/^"([^"]*)" has linked his Github account$/) do |name|
  ...
  @auth_identity.uid = '123456'
end

Given(/^"([^"]*)" has made (\d*) commits to "([^"]*)"$/) do |name, count, project_title|
  project =   Project.find_by_title('hello world')
  @github_response ||= {}
  @github_response[project_title] = File.read("spec/fixtures/github_#{project_title}_response.json")

  request = "http://api.github.com/repos/#{project.github_owner}/#{project.github_repo}/stats/contributors"
